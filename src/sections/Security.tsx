...
const ServiceCard = memo(({ service, idx, onClick }: { service: Feature; idx: number; onClick: () => void }) => {
...
const ServiceDetails = memo(({ service, onClose, onAction }: { service: Feature; onClose: () => void; onAction: () => void }) => {
  const Icon = iconMap[service.icon] || Shield;
  return (
    <>
...
const Security = () => {
  ...
  const securityServices = useMemo(() => 
    featuresConfig.features.filter(f => f.category === 'security'),
    []
  );
      ...
      <div className="section-container relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className={`${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'} transition-all duration-1000`}>
            <span className="inline-block mb-4 text-xs tracking-[0.4em] text-[#d4af37] font-medium uppercase text-glow">
              Private Security
            </span>
            <div className="gold-line mb-8" />
            <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl text-white mb-8 leading-tight">
              Discreet Protection <br />
              <span className="text-gradient-gold">For What Matters Most</span>
            </h2>
          ...
          <div className={`hidden lg:block relative ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'} transition-all duration-1000 delay-300`}>
            <div className="relative aspect-[4/5] overflow-hidden border border-[#d4af37]/20">
              <img 
                ...
                className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-[2000ms] scale-110 group-hover:scale-100"
              />
              <div className="absolute inset-0 bg-black/20" />
            </div>
            {/* Floating accent box */}
            <div className="absolute -bottom-10 -left-10 p-8 bg-[#111] border border-[#d4af37]/30 max-w-[280px] shadow-2xl">
              <div className="text-[#d4af37] font-serif text-4xl mb-2 animate-pulse-slow">24/7</div>
